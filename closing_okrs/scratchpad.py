...
def main():
    ...
    for table_name in ("subordinates", "managers", "teams", "users", "roles", "accounts", "user_roles"):
    ...
    print("Can a subordinate be a manager?")
    subordinates = set(tables["subordinates"]["userId.$oid"])
    managers = set(tables["managers"]["userId.$oid"])

    num_subordinates = len(subordinates)
    num_managers = len(managers)

    num_diff = len(managers - subordinates)

    if num_diff < num_managers:
        print(num_diff)
        print("yes")
    else:
        print("no")
    
    print("Is every user either in managers or subordinates?")
    print(tables["users"].columns.tolist())
    all_users = set(tables["users"]["_id.$oid"])
    subs_mans = subordinates | managers

    if len(all_users - subs_mans) == 0:
        print("yes")
    else:
        print("no")

    roles = tables["roles"]["name"].unique().tolist()

    for role in roles:
        _role = role.lower()
        if (
            "champion" in _role or
            ("okr" in _role and "champion" in _role)
        ):
            print(role)

    print("Accounts:")
    pprint(tables["accounts"].columns.tolist())
    print()
    print("Users:")
    pprint(tables["users"].columns.tolist())
    print()
    print("User Roles:")
    pprint(tables["user_roles"].columns.tolist())
    print(tables["user_roles"].head())

    df = tables["users"].merge(
        tables["teams"],
        how="left",
        left_on="userSettings.alignmentTreeFilter.teamIds",
        right_on="_id.$oid"
    )
