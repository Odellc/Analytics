...
def epoch_to_datetime(epoch):
    try:
        return datetime.datetime.fromtimestamp(0) + datetime.timedelta(milliseconds=epoch)
...
def get_users_data(blob_service_client, today_str):
    ...
    pprint(users_df.columns.tolist())
    print(
        [
            len(name)
            for name in users_df.columns.tolist()
            if "email" in name
        ]
    )
        ...
        users_df.columns[
            users_df.columns.isin(
                [
                    ...
                    'auth0Cache.givenname',
                    'auth0Cache.familyname',
                    'subscriptionType',
                    'isPrimary',
    ...
    print(users_df['auth0Cache.email'].head())

    # Drop rows containing users from Gtmhub or Prime Holding
    # users_df = users_df['auth0Cache.email'].fillna("-")
    users_df = users_df[~users_df['auth0Cache.email'].str.contains("gtmhub")]
    users_df = users_df[~users_df['auth0Cache.email'].str.contains("primeholding")]
