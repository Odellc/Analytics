...
def download(path, blob_client):
    ...
    with open(path, "wb") as json_file:
        for chunk in storage_stream_downloader.chunks():
...
if __name__ == "__main__":
    ...
    for table_name in event_table_names:
        ...
        if blob_size >= 20:
            ...
            download_path = os.path.join("data", "downloaded", file_name + ".json")
            download(download_path, blob_client)
            sys.exit()
    ...
    user_df = user_df.drop(['backenduser_id', 'backenduser_company_created_at', 'backenduser_last_name', 'backenduser_first_name', 'user_language'], axis=1)
    # Drop additional account columns
    account_df = account_df.drop(['account_language'], axis=1)

    # Join users and accounts
    _df = user_df.merge(account_df, how='inner', left_on='user_accountid', right_on='account_id')
