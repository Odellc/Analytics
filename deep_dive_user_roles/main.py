...
if __name__ == "__main__":
    ...
    df = df.merge(
        hubspot_df, how="left", left_on="user|email", right_on="hubspot|email"
    )
    ...
    print("=" * 40)
    print("user_roles_df")
    print(user_roles_df.columns)
    print(user_roles_df.shape)
    ...
    print("=" * 40)
    print("user_roles_df (joined)")
    print(user_roles_df.columns)
    print(user_roles_df.shape)

    # Get frequency of all roles
    role_frequency = (
        user_roles_df.groupby(["roles.name"])
        .size()
        .reset_index()
    ...
    print(role_frequency)
    print(role_frequency.columns)
    print(role_frequency["roles.name"].count())
    print(role_frequency["roles.name"].nunique())

    role_frequency.sort_values(by=["count"], inplace=True, ascending=False)

    print(role_frequency.head(20))

    role_frequency["new_role"] = role_frequency["roles.name"].map(assign_role)

    new_role_frequency = (
        role_frequency.groupby(["new_role"])
        .size()
        .reset_index()
        .rename(columns={0: "count"})
    )

    # Get percentage of model buy in
    total_usage = new_role_frequency["count"].sum()
    catchall_usage = new_role_frequency[new_role_frequency["new_role"] == "other"][
        "count"
    ].sum()

    print("Model Acceptance Rate:", (total_usage - catchall_usage) / total_usage)

    print(new_role_frequency.head(20))

    assigned_roles = ["executive", "admin", "okr", "manager", "hcm", "user"]

    print(role_frequency[~role_frequency["new_role"].isin(assigned_roles)])
