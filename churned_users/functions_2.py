...
def get_users_data(blob_service_client, today_str):
    # Get users
    print(f"Fetching user data for {today_str}...")
...
def get_users_accounts_subscriptions_data(blob_service_client, today_str):
    ...
    accounts_df = accounts_df[accounts_df["account|type"] == 1]

    print(accounts_df.shape)
    print(accounts_df["account|isActive"].unique().tolist())
    print(accounts_df.columns.tolist())
    print(accounts_df["account|type"].unique().tolist())
    pprint(accounts_df["chargebee|cancel_reason"].unique().tolist())
    sys.exit()
...
def get_roles_by_user(df):
    ...
    return df.groupby("user_roles.userId.$oid").apply(
        lambda x: ", ".join(
            str(role)
            for role in x["roles.name"].unique()
        )
    ).reset_index().rename(columns={0: "roles"})
